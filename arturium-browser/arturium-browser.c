...
{
    char *browser[] = {"python3 ./resources/plugin.py &> /tmp/arturium.log || echo $(cat /tmp/arturium.log). Exiting; rm /tmp/arturium.log 2>/dev/null"};
if (geteuid() == 0) {
    printf(" Cannot start %s with root user\n\r", argv[0]);
    exit(1);
}
